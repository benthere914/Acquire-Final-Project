...
def send_message():
    ...
    if len(message_boards) == 1:
        db.session.add(Message(messageBoardId=message_boards[0].to_dict()['id'], authorId=body['sellerId'], message=body['message'], createdAt=datetime.now()))
        db.session.commit()
    ...
    if len(message_boards) == 0:
        ...
        db.session.commit()
        db.session.add(Message(messageBoardId=message_board.id, authorId=body['sellerId'], message=body['message'], createdAt=datetime.now()))
        db.session.commit()
